module Magiika
  abstract class Node::Fn < TypeNodeClass::ClassTyping
    getter name : String
    ...
    def match_args(
        ...
        : {MatchResult, Hash(String, Psuedo::Node)?}
      anon_args = Array(Psuedo::Node).new
      kw_args = Hash(String, Psuedo::Node).new
      args.each do |arg|
        name = arg.name
        if name.nil?
          anon_args << arg.value
      ...
      arg_to_param_mapping = Hash(String, Psuedo::Node).new
      match_result = MatchResult.new

      @params.each do |param|
        p_name = param.name
        arg_data : Psuedo::Node? = nil
    ...
    def call_safe(
      ...
      type_node_args_hash = Hash(String, Psuedo::TypeNode).new
      node_args_hash.each { |key, value|
        type_value = value.eval(scope)
        unless type_value.is_a?(Psuedo::TypeNode)
    ...
    def call_safe_raise(
      ...
      return result.as(Psuedo::TypeNode)
    end
